...
| ツール | 説明 | 接続 |
|--------|------|:----:|
| `daily_brief` | 複数デバイスの朝次ヘルスチェックを並列実行。システム/シャーシアラーム、インターフェース Up/Down（ループバック除外）、指定時間内（`since_hours`、デフォルト 18 h）の syslog アラートパターンを確認し、CRITICAL/WARNING/OK の Markdown サマリーを返す | 要 |
