#!/usr/bin/env python3
"""Exercise every registered tool against the live devices and report what is broken.
...
Read-only by construction: every tool that changes a device — pushing config,
installing or rolling back a package, scheduling a reboot — is skipped by name
in ``smoke_probes.py``, and the report prints only tool names, statuses and row
counts, never payloads, which would otherwise carry configuration into logs.

Usage:
    # In-process: imports the server, uses its configured device inventory.
    JUNOS_OPS_CONFIG=... uv run python scripts/smoke_test.py
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        results = await run_probes(
            ...
            # Device data must not reach the report: every error here is
            # prefixed with the hostname it came from, and the payloads are
            # configuration. --traceback still shows the full text on the
            # operator's own terminal when debugging.
            redact_details=True,
        )
    finally:
...
def main() -> None:
    ...
    # Serial by default: these are production routers, several tools fan out
    # across the whole inventory already, and a NETCONF session is expensive to
    # set up — a burst of them is a worse neighbour than a slow smoke test.
