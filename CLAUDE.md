...
├── test_server.py              # 128 ユニットテスト
└── test_version_consistency.py # 2 ユニットテスト（バージョン整合性）
...
- 環境変数: `JUNOS_MCP_POOL`（無効化）・`JUNOS_MCP_POOL_IDLE`（アイドルタイムアウト秒、デフォルト 60）・`JUNOS_MCP_POOL_CONNECT_ATTEMPTS`（一過性接続失敗のリトライ回数、デフォルト 2＝1 回リトライ）・`JUNOS_MCP_POOL_CONNECT_DELAY`（リトライ間隔秒、デフォルト 1.0）
- 接続リトライの不変条件（PR #37）: リトライ対象は `_RETRYABLE_CONNECT_ERRORS`（`ConnectError` / `ConnectTimeoutError` のみ）に限定。`ConnectAuthError`（認証失敗）は**意図的に除外** — 誤った資格情報でリトライするとアカウントロックの恐れがあるため即座に fail-fast する。リトライの sleep は per-host `entry.lock` を保持したまま実行する（設計どおり — 同一ホストの呼び出しのみをブロックし他ホストは妨げない）
...
**スレッド安全上の注意:** これらプロセスグローバル状態はツール呼び出しごとに書き換えられる（`copy_package`/`install_package`/`rollback_package`/`schedule_reboot` は各呼び出し前に `common.args` の `dry_run`/`force`/`subcommand`/`unlink` をセット、`collect_rsi` は `common.config` の per-host `RSI_DIR` を一時書き換えて `finally` で復元）一方、バッチ系ツール（`run_show_command_batch`/`collect_rsi_batch`/`daily_brief`）は `common.run_parallel` でワーカーをスレッド実行する。したがって `common.args` を書き換える操作をバッチ化する新ツールや、ワーカー内で `common.config` を書き換えるコードは、無言のホスト間レースを招く。`daily_brief` が `host` オプションをスレッド起動前に事前設定するのは（"Pre-populate host option to avoid a ConfigParser race inside threads"）まさにこのレース回避のため。
...
157 テスト（グローバル初期化、config パス解決、接続プール、24 ツールの動作検証、バージョン整合性）。

`scripts/` にはライブスモークテストがある。`smoke_test.py`（CLI）、ツールごとの spec
`smoke_probes.py`、そしてサーバー非依存のエンジン `smoke_harness.py` — エンジンはこの
ハーネスを共有する各サーバーで同一に保つ方針なので、バグはこの写しを直すのではなく
一度直して全体に同期する。実機に対して登録ツールを全て実行する（README 参照）。
`tests/test_smoke_probes.py` はオフライン側の半分で、ツールレジストリだけで動く。
probe spec の無いツールを追加すると CI が落ちる: ツールを足す時点で「どうやって動作を
確認するか」を決めること。probe は機器を変えるツールには一切触れず、機器名・機種名を
直値で書かず（`args_factory` で実行時に取得）、`Error: ...` 行を拒否する
（さもないと到達不能な機器が成功として通る）。
